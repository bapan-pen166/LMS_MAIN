...
if(meetDayPattern=='CustomDay')
{
  handleWeekDaysOpen();
     ...
     console.log('schedule',schedule);
      if(!excelChk){
          ...
          axios.post(`${api}/video-call/create-meeting`,{
            meetings:[{startDate:start_datetime,
    ...
    const handleDailymeeting=(meeting)=>{
      ...
      axios.get(`${api}/video-call/get_meetings`)
      .then((Response) => {
                   ...
                   meetparticipate === 'Teacher' && 
                    <FormControl style={{ width: '230px', paddingLeft: '5px' }}>
                      ...
                      <Select
                        ...
                        input={<OutlinedInput label="Tag" />}
                        renderValue={(selected) => selected.map((mentor) => mentor.name).join(', ')}
                        sx={{ height: '40px', marginTop: '11px' }}
                      >
                        {mentorList.map((mentor) => (
                          <MenuItem key={mentor.id} value={mentor}>
                            <Checkbox checked={selectedmentor.some((selected) => selected.id === mentor.id)} />
                      ...
                      {/* {meetparticipate2 == 'Teacher' &&
                        ...
                        <FormControl style={{ width: '230px', paddingLeft: '5px' }}>
                          ...
                          <Select
                            ...
                            sx={{ height: '40px', marginTop: '11px' }}
                          >
                            {mentorList.map((option, index) => (
                              <MenuItem key={index} value={option.name}>
                                <Checkbox checked={selectedmentor.indexOf(option.name) > -1} />
                                <ListItemText primary={option.name} />
                              </MenuItem>
                            ))}
                          </Select>
